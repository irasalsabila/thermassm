...
def load_series(cfg) -> tuple[np.ndarray, np.ndarray]:
    if cfg.data.use_synthetic:
        ...
        nc_dir = cfg.data.data_dir
        if not __import__("os").path.isdir(
            __import__("os").path.join(nc_dir, "weatherbench_data")
        ):
            from .download import download_weatherbench_netcdf

            nc_dir = download_weatherbench_netcdf(nc_dir)
        dates, t2m = extract_point_series(
            nc_dir, cfg.data.lat, cfg.data.lon,
